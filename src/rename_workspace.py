...
def rename_workspace(workspace_name: str):
    sway.command(f"rename workspace to {workspace_name}")
    Gtk.main_quit()
...
class WorkspaceRename(Gtk.Window):
    ...
    def __init__(self):
        ...
        self.rename_entry.set_text(get_focused_workspace())
        self.rename_entry.connect("activate", self.rename_entry_activate)
        self.rename_entry.connect("key-press-event", self.rename_entry_key_pressed)
    ...
    def rename_entry_key_pressed(self, _widget, event) -> bool:
        pass
